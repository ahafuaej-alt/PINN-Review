...
| CR000276 | The live HYPERVIEW page and primary Atlas paper 128 verify the exact paper-dataset relationship. The authoritative Stage-1 PRL registry already contains `PRL000024` for CR000276 ↔ paper 128. Dataset use is governed by non-commercial research-use and required-citation terms rather than an inferred SPDX license. | `PRL000024` verified as `paper_dataset_mention`. Linked starter material is separately pinned but remains supporting evidence. Dataset-file, spectral/target-schema, permanent-successor-record, and notebook normalization remain Stage 3 work. |
